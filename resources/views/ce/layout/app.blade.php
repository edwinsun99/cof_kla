...
<body>
    ...
    <nav class="sidebar" id="mySidebar">
        <a href="{{ route('ce.home') }}" class="menu-item mt-2 {{ request()->routeIs('ce.home') ? 'active' : '' }}">
            <i class="bi bi-house-door"></i> <span>Home</span>
        </a>

        <div class="accordion accordion-flush" id="sidebarAccordion">
            <div class="accordion-item border-0 bg-transparent">
                @php $caseActive = request()->routeIs('ce.services.*') || request()->routeIs('cases.*') || request()->routeIs('ce.finish.*'); @endphp
                <a href="#menuCases" class="menu-item {{ $caseActive ? 'active' : '' }}" data-bs-toggle="collapse">
                    <i class="bi bi-folder"></i> <span>Cases Management</span>
                </a>
                <div id="menuCases" class="collapse {{ $caseActive ? 'show' : '' }}" data-bs-parent="#sidebarAccordion">
                    <div class="submenu-container">
                        <a href="{{ route('ce.services.index') }}" class="submenu-item {{ request()->routeIs('ce.services.index') ? 'active-sub' : '' }}">View All Cases</a>
                        <a href="{{ route('cases.new') }}" class="submenu-item {{ request()->routeIs('cases.new') ? 'active-sub' : '' }}">New Case</a>
                        <a href="{{ route('ce.finish.repair') }}" class="submenu-item {{ request()->routeIs('ce.finish.repair') ? 'active-sub' : '' }}">Finish Repair</a>
                    </div>
                </div>
            </div>
        </div>

        <a href="{{ route('ce.engineer.index') }}" class="menu-item {{ request()->routeIs('ce.engineer.index') ? 'active' : '' }}">
            <i class="bi bi-wrench-adjustable"></i> <span>Engineer Cases</span>
        </a>
        <a href="{{ route('ce.quotation.appcancl') }}" class="menu-item {{ request()->routeIs('ce.quotation.appcancl') ? 'active' : '' }}">
            <i class="bi bi-gem"></i> <span>Quotation Status</span>
        </a>
        <a href="{{ route('erf.select') }}" class="menu-item {{ request()->routeIs('erf.select') ? 'active' : '' }}">
